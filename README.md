...
![S3 structure](docs/S3-bucket-structure.png)
![Glue batch run](docs/glue-batch-to-curated.png)
![Glue incremental run](docs/glue-incremental-to-curated.png)
![Athena query result](docs/Athena-query-result.png)
![Glue tables](docs/glue-tables-lakehouse-db.png)
![EventBridge schedule](docs/eventbridge-schedule.png)
![Lambda function](docs/lambda-function.png)

## Results

- Successfully built batch and incremental data pipelines on AWS.
- Curated data written as Parquet with partition strategy (`year/month/day`).
- Athena analytics queries executed successfully (daily revenue, top products, customer LTV, region performance).
- Incremental files were generated by Lambda and processed into curated outputs.
